...
import { Edit, Lock, Search, Trash2, X, ShieldCheck, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function ComplianceCategories() {
    ...
    const handleResetFilters = () => {
        ...
        router.get(route('compliance.categories.index'), {}, { preserveState: true, preserveScroll: true });
    };
    ...
    const handleFormSubmit = (e: React.FormEvent) => {
        ...
        if (formMode === 'create') {
            router.post(route('compliance.categories.store'), formData, {
                ...
                onError: (errors) => { setFormErrors(errors); toast.error(t('Failed to create compliance category.')); },
            });
        } else {
            router.put(route('compliance.categories.update', currentItem.id), formData, {
                onSuccess: (page) => { if (page.props.flash.success) { toast.success(page.props.flash.success); resetForm(); } else if (page.props.flash.error) toast.error(page.props.flash.error); },
                onError: (errors) => { setFormErrors(errors); toast.error(t('Failed to update compliance category.')); },
            });
        }
    };
    ...
    const handleDeleteConfirm = () => {
        router.delete(route('compliance.categories.destroy', currentItem.id), {
            ...
            onError: (errors) => { setIsDeleteModalOpen(false); toast.error(`${t('Failed to delete compliance category')}: ${Object.values(errors).join(', ')}`); },
        });
    };
    ...
    const handleToggleStatus = (item: any) => {
        router.put(route('compliance.categories.toggle-status', item.id), {}, {
            ...
            onError: (errors) => toast.error(`${t('Failed to update compliance category')}: ${Object.values(errors).join(', ')}`),
        });
    };
    ...
    const canCreate = hasPermission(permissions, 'create-compliance-categories');
    const canEdit = hasPermission(permissions, 'edit-compliance-categories');
    const canDelete = hasPermission(permissions, 'delete-compliance-categories');
    const canToggle = hasPermission(permissions, 'toggle-status-compliance-categories');
    ...
    return (
        <PageTemplate title={t('Compliance Categories')} description={t('Manage compliance categories for your organization.')} url="/compliance/categories" breadcrumbs={breadcrumbs} noPadding>
                    ...
                    <div className="sticky top-4 rounded-lg border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
                        <div className="border-b border-gray-200 p-6 dark:border-gray-700">
                            ...
                            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                                {formMode === 'create' ? t('Fill in the details to create a new compliance category') : t('Update the compliance category details below')}
                            </p>
                        </div>
                        ...
                        <form onSubmit={handleFormSubmit} className="space-y-4 p-6">
                            <div className="space-y-2">
                                ...
                                <Input id="name" type="text" value={formData.name}
                                    ...
                                    placeholder={t('eg. Data Privacy, Financial')}
                        ...
                        <div className="space-y-4">
                            <div className="flex gap-2">
                                <div className="relative flex-1">
                                    ...
                                    <Input type="text" placeholder={t('Search categories...')} value={searchTerm}
                        ...
                        {(categories?.data || []).length > 0 ? (
                            <>
                                {/* <div className="border-b border-gray-200 px-6 py-4 dark:border-gray-700">
                                    <h3 className="text-lg font-semibold text-gray-900 dark:text-white">{t('Compliance Categories')}</h3>
                                    <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">{t('Manage compliance categories for your organization.')}</p>
                                </div> */}
                                ...
                                {categories?.total > (categories?.per_page || 10) && (
                                    <div className="border-t border-gray-200 dark:border-gray-700">
                                        <Pagination
                                            ...
                                            entityName={t('compliance categories')}
                                            hidePerPage={true}
                            ...
                            <div className="p-12 text-center">
                                ...
                                <h3 className="mb-2 text-lg font-medium text-gray-900 dark:text-white">{t('No compliance categories found')}</h3>
                                <p className="mx-auto mb-6 max-w-sm text-gray-500 dark:text-gray-400">
                                    {hasActiveFilters() ? t('No compliance categories match your search criteria. Try adjusting your filters.') : t('Create compliance categories to organize your compliance items.')}
                                </p>
                                {!hasActiveFilters() && canCreate && <p className="text-sm text-gray-500 dark:text-gray-400">{t('Use the form on the left to add your first compliance category.')}</p>}
                            </div>
                        )}
                    </div>
                </div>
            ...
            <CrudDeleteModal
                ...
                entityName={t('compliance category')}
            />
        </PageTemplate>
    );
}
