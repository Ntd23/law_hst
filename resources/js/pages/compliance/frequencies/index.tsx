...
import { Edit, Lock, Search, Trash2, X, Clock, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function ComplianceFrequencies() {
    ...
    }, [checkOverflow, frequencies?.data]);

    const [formData, setFormData] = useState({ name: '', description: '', days: '', status: 'active' });
    const [formErrors, setFormErrors] = useState<any>({});

    const resetForm = () => {
        setFormData({ name: '', description: '', days: '', status: 'active' });
        setFormErrors({}); setFormMode('create'); setCurrentItem(null);
    };

    const loadItemForEdit = (item: any) => {
        setFormData({ name: item.name || '', description: item.description || '', days: item.days ? String(item.days) : '', status: item.status || 'active' });
    ...
    const handleSort = (field: string) => {
        ...
        router.get(route('compliance.frequencies.index'), {
            ...
            search: searchTerm || undefined, status: selectedStatus !== 'all' ? selectedStatus : undefined,
            per_page: pageFilters.per_page || 10,
        }, { preserveState: true, preserveScroll: true });
    };
    ...
    const handleResetFilters = () => {
        ...
        router.get(route('compliance.frequencies.index'), {}, { preserveState: true, preserveScroll: true });
    };
    ...
    const handleFormSubmit = (e: React.FormEvent) => {
        ...
        const payload = { ...formData, days: formData.days ? Number(formData.days) : null };
        if (formMode === 'create') {
            router.post(route('compliance.frequencies.store'), payload, {
                ...
                onError: (errors) => { setFormErrors(errors); toast.error(t('Failed to create frequency.')); },
            });
        } else {
            router.put(route('compliance.frequencies.update', currentItem.id), payload, {
                onSuccess: (page) => { if (page.props.flash.success) { toast.success(page.props.flash.success); resetForm(); } else if (page.props.flash.error) toast.error(page.props.flash.error); },
                onError: (errors) => { setFormErrors(errors); toast.error(t('Failed to update frequency.')); },
            });
        }
    };
    ...
    const handleDeleteConfirm = () => {
        router.delete(route('compliance.frequencies.destroy', currentItem.id), {
            ...
            onError: (errors) => { setIsDeleteModalOpen(false); toast.error(`${t('Failed to delete frequency')}: ${Object.values(errors).join(', ')}`); },
        });
    };
    ...
    const handleToggleStatus = (item: any) => {
        router.put(route('compliance.frequencies.toggle-status', item.id), {}, {
            ...
            onError: (errors) => toast.error(`${t('Failed to update frequency')}: ${Object.values(errors).join(', ')}`),
        });
    };
    ...
    const getDays = (days: number | null) => {
        ...
        switch (days) {
            case 1: return t('Daily');
            case 7: return t('Weekly');
            case 30: return t('Monthly');
            case 90: return t('Quarterly');
            case 180: return t('Semi-Annually');
            case 365: return t('Annually');
            default: return `${days} ${t('days')}`;
        }
    };
    ...
    const canCreate = hasPermission(permissions, 'create-compliance-frequencies');
    const canEdit = hasPermission(permissions, 'edit-compliance-frequencies');
    const canDelete = hasPermission(permissions, 'delete-compliance-frequencies');
    const canToggle = hasPermission(permissions, 'toggle-status-compliance-frequencies');
    ...
    return (
        <PageTemplate title={t('Compliance Frequencies')} description={t('Define recurring schedules for compliance tasks.')} url="/compliance/frequencies" breadcrumbs={breadcrumbs} noPadding>
                    ...
                    <div className="sticky top-4 rounded-lg border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
                        <div className="border-b border-gray-200 p-6 dark:border-gray-700">
                            <h2 className="text-lg font-semibold text-gray-900 dark:text-white">
                                {formMode === 'create' ? t('Add New Frequency') : t('Edit Frequency')}
                            </h2>
                            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                                {formMode === 'create' ? t('Fill in the details to create a new compliance frequency') : t('Update the compliance frequency details below')}
                            </p>
                        </div>

                        <form onSubmit={handleFormSubmit} className="space-y-4 p-6">
                            <div className="space-y-2">
                                <Label htmlFor="name" required>{t('Frequency Name')}</Label>
                                <Input id="name" type="text" value={formData.name}
                                    onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                                    placeholder={t('eg. Daily, Monthly, Quarterly')}
                            ...
                            <div className="space-y-2">
                                ...
                                <Input id="days" type="number" min="1" value={formData.days}
                                    onChange={(e) => setFormData({ ...formData, days: e.target.value })}
                                    placeholder={t('eg. 30')}
                                    className={formErrors.days ? 'border-red-500' : ''}
                                    disabled={!canCreate && !canEdit} />
                                {formErrors.days && <p className="text-sm text-red-500">{formErrors.days}</p>}
                            </div>
                            ...
                            <div className="space-y-2">
                                ...
                                <Textarea id="description" value={formData.description}
                                    ...
                                    placeholder={t('Enter frequency description...')}
                            ...
                            <div className="flex items-center gap-3 border-t border-gray-200 pt-4 dark:border-gray-700">
                                {(canCreate || canEdit) && (
                                    <Button type="submit" className="flex-1">
                                        {formMode === 'create' ? t('Add Frequency') : t('Update Frequency')}
                                    </Button>
                                )}
                        ...
                        <div className="space-y-4">
                            <div className="flex gap-2">
                                <div className="relative flex-1">
                                    ...
                                    <Input type="text" placeholder={t('Search frequencies...')} value={searchTerm}
                        ...
                        {(frequencies?.data || []).length > 0 ? (
                            <>
                                {/* <div className="border-b border-gray-200 px-6 py-4 dark:border-gray-700">
                                    <h3 className="text-lg font-semibold text-gray-900 dark:text-white">{t('Compliance Frequencies')}</h3>
                                    <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">{t('Manage compliance frequencies for your organization.')}</p>
                                </div> */}
                                    ...
                                    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                        <thead className="bg-gray-50 dark:bg-gray-700">
                                            <tr className='bg-[#F0F0F1] hover:bg-[#F0F0F1] dark:border-gray-900 dark:bg-gray-900 border-t'>
                                                ...
                                                <th className="cursor-pointer px-3 py-3 text-left text-xs font-medium tracking-wider text-gray-500 select-none dark:text-gray-300" onClick={() => handleSort('days')}>
                                                    <div className="flex items-center gap-1">
                                                        {t('Days')}
                                                        {pageFilters.sort_field === 'days' ? (pageFilters.sort_direction === 'asc' ? ' ↑' : ' ↓') : <span className="opacity-40">↕</span>}
                                                    </div>
                                                </th>
                                            ...
                                            {frequencies.data.map((item: any) => (
                                                <tr key={item.id} className="transition-colors hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                                    ...
                                                    <td className="px-3 py-4">
                                                        <span className="inline-flex items-center rounded-md px-2 py-1 text-xs font-medium ring-1 ring-inset bg-purple-50 text-purple-700 ring-purple-600/20">{getDays(item.days)}</span>
                                                    </td>
                                                    <td className="px-3 py-4">
                                    ...
                                    {frequencies.data.map((item: any) => (
                                        <div key={item.id} className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                                            ...
                                            <div className="mt-3 grid grid-cols-2 gap-3 border-t border-gray-100 pt-3 dark:border-gray-700">
                                                <div>
                                                    <p className="mb-1 text-xs text-gray-500 dark:text-gray-400">{t('Days')}</p>
                                                    <span className="inline-flex items-center rounded-md px-2 py-1 text-xs font-medium ring-1 ring-inset bg-purple-50 text-purple-700 ring-purple-600/20">{getDays(item.days)}</span>
                                                </div>
                                                <div>
                                ...
                                {frequencies?.total > (frequencies?.per_page || 10) && (
                                    <div className="border-t border-gray-200 dark:border-gray-700">
                                        <Pagination
                                            from={frequencies?.from || 0} to={frequencies?.to || 0}
                                            total={frequencies?.total || 0} links={frequencies?.links}
                            ...
                            <div className="p-12 text-center">
                                ...
                                <h3 className="mb-2 text-lg font-medium text-gray-900 dark:text-white">{t('No frequencies found')}</h3>
                                <p className="mx-auto mb-6 max-w-sm text-gray-500 dark:text-gray-400">
                                    {hasActiveFilters() ? t('No frequencies match your search criteria. Try adjusting your filters.') : t('Create compliance frequencies to schedule your compliance tasks.')}
                                </p>
                                {!hasActiveFilters() && canCreate && <p className="text-sm text-gray-500 dark:text-gray-400">{t('Use the form on the left to add your first frequency.')}</p>}
                            </div>
                        )}
                    </div>
                </div>
