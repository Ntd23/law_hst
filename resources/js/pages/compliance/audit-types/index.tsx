...
import { Edit, Lock, Search, Trash2, X, ClipboardCheck, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function AuditTypes() {
    ...
    const handleResetFilters = () => {
        ...
        router.get(route('compliance.audit-types.index'), {}, { preserveState: true, preserveScroll: true });
    };
    ...
    const handleFormSubmit = (e: React.FormEvent) => {
        ...
        if (formMode === 'create') {
            router.post(route('compliance.audit-types.store'), formData, {
                ...
                onError: (errors) => { setFormErrors(errors); toast.error(t('Failed to create audit type.')); },
            });
        } else {
            router.put(route('compliance.audit-types.update', currentItem.id), formData, {
                onSuccess: (page) => { if (page.props.flash.success) { toast.success(page.props.flash.success); resetForm(); } else if (page.props.flash.error) toast.error(page.props.flash.error); },
                onError: (errors) => { setFormErrors(errors); toast.error(t('Failed to update audit type.')); },
            });
        }
    };
    ...
    const handleDeleteConfirm = () => {
        router.delete(route('compliance.audit-types.destroy', currentItem.id), {
            ...
            onError: (errors) => { setIsDeleteModalOpen(false); toast.error(`${t('Failed to delete audit type')}: ${Object.values(errors).join(', ')}`); },
        });
    };
    ...
    const handleToggleStatus = (item: any) => {
        router.put(route('compliance.audit-types.toggle-status', item.id), {}, {
            ...
            onError: (errors) => toast.error(`${t('Failed to update audit type')}: ${Object.values(errors).join(', ')}`),
        });
    };
    ...
    const canCreate = hasPermission(permissions, 'create-audit-types');
    const canEdit = hasPermission(permissions, 'edit-audit-types');
    const canDelete = hasPermission(permissions, 'delete-audit-types');
    const canToggle = hasPermission(permissions, 'toggle-status-audit-types');
    ...
    return (
        <PageTemplate title={t('Audit Types')} description={t('Define and manage audit type classifications.')} url="/compliance/audit-types" breadcrumbs={breadcrumbs} noPadding>
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
                    ...
                    <div className="sticky top-4 rounded-lg border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
                        <div className="border-b border-gray-200 p-6 dark:border-gray-700">
                            <h2 className="text-lg font-semibold text-gray-900 dark:text-white">
                                {formMode === 'create' ? t('Add New Audit Type') : t('Edit Audit Type')}
                            </h2>
                            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                                {formMode === 'create' ? t('Fill in the details to create a new audit type') : t('Update the audit type details below')}
                            </p>
                        </div>

                        <form onSubmit={handleFormSubmit} className="space-y-4 p-6">
                            <div className="space-y-2">
                                <Label htmlFor="name" required>{t('Audit Type Name')}</Label>
                                <Input id="name" type="text" value={formData.name}
                                    onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                                    placeholder={t('eg. Internal, External')}
                            ...
                            <div className="space-y-2">
                                ...
                                <Textarea id="description" value={formData.description}
                                    ...
                                    placeholder={t('Enter audit type description...')}
                            ...
                            <div className="flex items-center gap-3 border-t border-gray-200 pt-4 dark:border-gray-700">
                                {(canCreate || canEdit) && (
                                    <Button type="submit" className="flex-1">
                                        {formMode === 'create' ? t('Add Audit Type') : t('Update Audit Type')}
                                    </Button>
                                )}
                ...
                <div className="space-y-4 lg:col-span-2">
                        ...
                        <div className="space-y-4">
                            <div className="flex gap-2">
                                <div className="relative flex-1">
                                    ...
                                    <Input type="text" placeholder={t('Search audit types...')} value={searchTerm}
                    ...
                    <div className="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
                        {(auditTypes?.data || []).length > 0 ? (
                            <>
                                {/* <div className="border-b border-gray-200 px-6 py-4 dark:border-gray-700">
                                    <h3 className="text-lg font-semibold text-gray-900 dark:text-white">{t('Audit Types')}</h3>
                                    <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">{t('Manage audit type categories for your organization.')}</p>
                                </div> */}
                                ...
                                {auditTypes?.total > (auditTypes?.per_page || 10) && (
                                    <div className="border-t border-gray-200 dark:border-gray-700">
                                        <Pagination
                                            from={auditTypes?.from || 0} to={auditTypes?.to || 0}
                                            total={auditTypes?.total || 0} links={auditTypes?.links}
                            ...
                            <div className="p-12 text-center">
                                ...
                                <h3 className="mb-2 text-lg font-medium text-gray-900 dark:text-white">{t('No audit types found')}</h3>
                                <p className="mx-auto mb-6 max-w-sm text-gray-500 dark:text-gray-400">
                                    {hasActiveFilters() ? t('No audit types match your search criteria. Try adjusting your filters.') : t('Create audit types to categorize your compliance audits.')}
                                </p>
                                {!hasActiveFilters() && canCreate && <p className="text-sm text-gray-500 dark:text-gray-400">{t('Use the form on the left to add your first audit type.')}</p>}
                            </div>
                        )}
                    </div>
                </div>
