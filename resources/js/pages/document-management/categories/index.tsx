...
import { Edit, Lock, Search, Trash2, X, FolderOpen, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function DocumentCategories() {
    ...
    const handleResetFilters = () => {
        ...
        router.get(route('document-management.categories.index'), {}, { preserveState: true, preserveScroll: true });
    };
    ...
    const handleFormSubmit = (e: React.FormEvent) => {
        ...
        if (formMode === 'create') {
            router.post(route('document-management.categories.store'), formData, {
                ...
                onError: (errors) => { setFormErrors(errors); toast.error(t('Failed to create document category.')); },
            });
        } else {
            router.put(route('document-management.categories.update', currentItem.id), formData, {
                onSuccess: (page) => { if (page.props.flash.success) { toast.success(page.props.flash.success); resetForm(); } else if (page.props.flash.error) toast.error(page.props.flash.error); },
                onError: (errors) => { setFormErrors(errors); toast.error(t('Failed to update document category.')); },
            });
        }
    };
    ...
    const handleDeleteConfirm = () => {
        router.delete(route('document-management.categories.destroy', currentItem.id), {
            ...
            onError: (errors) => { setIsDeleteModalOpen(false); toast.error(`${t('Failed to delete document category')}: ${Object.values(errors).join(', ')}`); },
        });
    };

    const handleToggleStatus = (item: any) => {
        router.put(route('document-management.categories.toggle-status', item.id), {}, {
            onSuccess: (page) => {
            ...
            onError: (errors) => toast.error(`${t('Failed to update document category')}: ${Object.values(errors).join(', ')}`),
        });
    };
    ...
    const canToggle = hasPermission(permissions, 'toggle-status-document-categories');
    ...
    return (
        <PageTemplate title={t('Document Categories')} description={t('Manage document categories for your organization.')} url="/document-management/categories" breadcrumbs={breadcrumbs} noPadding>
                    ...
                    <div className="sticky top-4 rounded-lg border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
                        <div className="border-b border-gray-200 p-6 dark:border-gray-700">
                            ...
                            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                                {formMode === 'create' ? t('Fill in the details to create a new document category') : t('Update the document category details below')}
                            </p>
                        </div>
                        ...
                        <form onSubmit={handleFormSubmit} className="space-y-4 p-6">
                            <div className="space-y-2">
                                ...
                                <Input id="name" type="text" value={formData.name}
                                    ...
                                    placeholder={t('eg. Contracts, Pleadings')}
                        ...
                        {(categories?.data || []).length > 0 ? (
                            <>
                                {/* <div className="border-b border-gray-200 px-6 py-4 dark:border-gray-700">
                                    ...
                                    <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">{t('Manage document categories for your organization.')}</p>
                                </div> */}
                            ...
                            <div className="p-12 text-center">
                                ...
                                <p className="mx-auto mb-6 max-w-sm text-gray-500 dark:text-gray-400">
                                    {hasActiveFilters() ? t('No document categories match your search criteria. Try adjusting your filters.') : t('Create document categories to organize your documents.')}
                                </p>
                                {!hasActiveFilters() && canCreate && <p className="text-sm text-gray-500 dark:text-gray-400">{t('Use the form on the left to add your first document category.')}</p>}
                            </div>
                        )}
                    </div>
                </div>
            ...
            <CrudDeleteModal
                ...
                entityName={t('document category')}
            />
        </PageTemplate>
    );
}
