...
import { Edit, Lock, Search, Trash2, X, BookOpen, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function ResearchTypes() {
    ...
    }, [checkOverflow, researchTypes?.data]);
    ...
    const handleResetFilters = () => {
        ...
        router.get(route('legal-research.research-types.index'), {}, { preserveState: true, preserveScroll: true });
    };
    ...
    const handleFormSubmit = (e: React.FormEvent) => {
        ...
        if (formMode === 'create') {
            router.post(route('legal-research.research-types.store'), formData, {
                ...
                onError: (errors) => { setFormErrors(errors); toast.error(t('Failed to create research type.')); },
            });
        } else {
            router.put(route('legal-research.research-types.update', currentItem.id), formData, {
                onSuccess: (page) => { if (page.props.flash.success) { toast.success(page.props.flash.success); resetForm(); } else if (page.props.flash.error) toast.error(page.props.flash.error); },
                onError: (errors) => { setFormErrors(errors); toast.error(t('Failed to update research type.')); },
            });
        }
    };
    ...
    const handleDeleteConfirm = () => {
        router.delete(route('legal-research.research-types.destroy', currentItem.id), {
            ...
            onError: (errors) => { setIsDeleteModalOpen(false); toast.error(`${t('Failed to delete research type')}: ${Object.values(errors).join(', ')}`); },
        });
    };

    const handleToggleStatus = (item: any) => {
        router.put(route('legal-research.research-types.toggle-status', item.id), {}, {
            onSuccess: (page) => {
            ...
            onError: (errors) => toast.error(`${t('Failed to update research type')}: ${Object.values(errors).join(', ')}`),
        });
    };
    ...
    const canCreate = hasPermission(permissions, 'create-research-types');
    const canEdit = hasPermission(permissions, 'edit-research-types');
    const canToggle = hasPermission(permissions, 'toggle-status-research-types');
    const canDelete = hasPermission(permissions, 'delete-research-types');
    ...
    return (
        <PageTemplate title={t('Research Types')} url="/legal-research/research-types" breadcrumbs={breadcrumbs} description={t('Manage research type categories for your organization.')} noPadding>
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
                    ...
                    <div className="sticky top-4 rounded-lg border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
                        <div className="border-b border-gray-200 p-6 dark:border-gray-700">
                            <h2 className="text-lg font-semibold text-gray-900 dark:text-white">
                                {formMode === 'create' ? t('Add New Research Type') : t('Edit Research Type')}
                            </h2>
                            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                                {formMode === 'create' ? t('Fill in the details to create a new research type') : t('Update the research type details below')}
                            </p>
                        </div>

                        <form onSubmit={handleFormSubmit} className="space-y-4 p-6">
                            <div className="space-y-2">
                                <Label htmlFor="name" required>{t('Research Type Name')}</Label>
                                <Input id="name" type="text" value={formData.name}
                                    onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                                    placeholder={t('eg. Case Law, Statutory')}
                            ...
                            <div className="space-y-2">
                                ...
                                <Textarea id="description" value={formData.description}
                                    ...
                                    placeholder={t('Enter research type description...')}
                            ...
                            <div className="flex items-center gap-3 border-t border-gray-200 pt-4 dark:border-gray-700">
                                {(canCreate || canEdit) && (
                                    <Button type="submit" className="flex-1">
                                        {formMode === 'create' ? t('Add Research Type') : t('Update Research Type')}
                                    </Button>
                                )}
                ...
                <div className="space-y-4 lg:col-span-2">
                        ...
                        <div className="space-y-4">
                            <div className="flex gap-2">
                                <div className="relative flex-1">
                                    ...
                                    <Input type="text" placeholder={t('Search research types...')} value={searchTerm}
                    ...
                    <div className="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
                        {(researchTypes?.data || []).length > 0 ? (
                            <>
                                {/* <div className="border-b border-gray-200 px-6 py-4 dark:border-gray-700">
                                    <h3 className="text-lg font-semibold text-gray-900 dark:text-white">{t('Research Types')}</h3>
                                    <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">{t('Manage research type categories for your organization.')}</p>
                                </div> */}
                                ...
                                <div className="hidden overflow-x-auto lg:block">
                                    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                        ...
                                        <tbody className="divide-y divide-gray-200 bg-white dark:divide-gray-700 dark:bg-gray-800">
                                            {researchTypes.data.map((item: any) => (
                                ...
                                {researchTypes?.total > (researchTypes?.per_page || 10) && (
                                    <div className="border-t border-gray-200 dark:border-gray-700">
                                        <Pagination
                                            from={researchTypes?.from || 0} to={researchTypes?.to || 0}
                                            total={researchTypes?.total || 0} links={researchTypes?.links}
                            ...
                            <div className="p-12 text-center">
                                ...
                                <h3 className="mb-2 text-lg font-medium text-gray-900 dark:text-white">{t('No research types found')}</h3>
                                <p className="mx-auto mb-6 max-w-sm text-gray-500 dark:text-gray-400">
                                    {hasActiveFilters() ? t('No research types match your search criteria. Try adjusting your filters.') : t('Create research types to categorize your legal research.')}
                                </p>
                                {!hasActiveFilters() && canCreate && <p className="text-sm text-gray-500 dark:text-gray-400">{t('Use the form on the left to add your first research type.')}</p>}
                            </div>
                        )}
                    </div>
                </div>
