...
export default function ResearchCategories() {
    ...
    const { auth, categories, practiceAreas, allPracticeAreas, filters: pageFilters = {} } = usePage().props as any;
    ...
    const [formData, setFormData] = useState({
        name: '', description: '', color: '#3b82f6', practice_area_id: '', status: 'active',
    });
    const [formErrors, setFormErrors] = useState<any>({});

    const resetForm = () => {
        setFormData({ name: '', description: '', color: '#3b82f6', practice_area_id: '', status: 'active' });
        setFormErrors({});
        setFormMode('create');
        setCurrentItem(null);
    };
    ...
    const loadItemForEdit = (item: any) => {
        setFormData({
            ...
            practice_area_id: item.practice_area_id?.toString() || '',
    ...
    const hasActiveFilters = () => searchTerm !== '' || selectedPracticeArea !== 'all' || selectedStatus !== 'all';
    ...
    const handleResetFilters = () => {
        ...
        router.get(route('legal-research.categories.index'), {}, { preserveState: true, preserveScroll: true });
    };
    ...
    const handleFormSubmit = (e: React.FormEvent) => {
        ...
        if (formMode === 'create') {
            router.post(route('legal-research.categories.store'), formData, {
                onSuccess: (page) => {
                    toast.dismiss();
                    if (page.props.flash.success) { toast.success(page.props.flash.success); resetForm(); }
                    if (page.props.flash.error) toast.error(page.props.flash.error);
                },
                onError: (errors) => { setFormErrors(errors); toast.error(`Failed to store research category: ${Object.values(errors).join(', ')}`); },
            });
        } else {
            router.put(route('legal-research.categories.update', currentItem.id), formData, {
                onSuccess: (page) => {
                    toast.dismiss();
                    if (page.props.flash.success) { toast.success(page.props.flash.success); resetForm(); }
                    if (page.props.flash.error) toast.error(page.props.flash.error);
                },
                onError: (errors) => { setFormErrors(errors); toast.error(`Failed to update research category: ${Object.values(errors).join(', ')}`); },
            });
        }
    };
    ...
    const handleDeleteConfirm = () => {
        router.delete(route('legal-research.categories.destroy', currentItem.id), {
            ...
            onError: (errors) => { setIsDeleteModalOpen(false); toast.error(`Failed to delete research category: ${Object.values(errors).join(', ')}`); },
        });
    };
        ...
        router.put(route('legal-research.categories.toggle-status', category.id), {}, {
            onSuccess: (page) => {
                if (page.props.flash.success) {
                    ...
                    if (formMode === 'edit' && currentItem?.id === category.id)
                        setFormData(prev => ({ ...prev, status: category.status === 'active' ? 'inactive' : 'active' }));
                }
                if (page.props.flash.error) toast.error(page.props.flash.error);
            },
            onError: (errors) => { toast.error(`Failed to update category status: ${Object.values(errors).join(', ')}`); },
        });
    };
    ...
    const canCreate = hasPermission(permissions, 'create-research-categories');
    const canEdit = hasPermission(permissions, 'edit-research-categories');
    const canToggle = hasPermission(permissions, 'toggle-status-research-categories');
    const canDelete = hasPermission(permissions, 'delete-research-categories');
    ...
    return (
        <PageTemplate title={t('Research Categories')} url="/legal-research/categories" description={t('Manage research categories for your organization.')} breadcrumbs={breadcrumbs} noPadding>
                    ...
                    <div className="sticky top-4 rounded-lg border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
                        <div className="border-b border-gray-200 p-6 dark:border-gray-700">
                            <h2 className="text-lg font-semibold text-gray-900 dark:text-white">
                                {formMode === 'create' ? t('Add New Research Category') : t('Edit Research Category')}
                            </h2>
                            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                                {formMode === 'create' ? t('Fill in the details to create a new research category') : t('Update the research category details below')}
                            </p>
                        </div>
                        ...
                        <form onSubmit={handleFormSubmit} className="space-y-4 p-6">
                            <div className="space-y-2">
                                ...
                                <Input id="name" type="text" value={formData.name}
                                    ...
                                    placeholder={t('eg. Constitutional Law')}
                            ...
                            <div className="space-y-2">
                                ...
                                <Textarea id="description" value={formData.description}
                                    ...
                                    placeholder={t('eg. Research related to constitutional rights and interpretations')}
                            ...
                            <div className="space-y-2">
                                <Label htmlFor="practice_area_id" required>{t('Practice Area')}</Label>
                                <Select value={formData.practice_area_id} onValueChange={(value) => setFormData({ ...formData, practice_area_id: value })} disabled={!canCreate && !canEdit}>
                                    <SelectTrigger className={formErrors.practice_area_id ? 'border-red-500' : ''}>
                                        <SelectValue placeholder={t('Select practice area')} />
                                    </SelectTrigger>
                                    <SelectContent searchable={true}>
                                        {(practiceAreas || []).length === 0 ? (
                                            <div className="px-3 py-2 text-sm text-gray-500">
                                                {t('No practice areas found.')} <a href={route('advocate.practice-areas.index')} className="text-blue-600 underline">{t('Practice Areas')}</a>
                                            </div>
                                        ) : (
                                ...
                                {formErrors.practice_area_id && <p className="text-sm text-red-500">{formErrors.practice_area_id}</p>}
                            </div>
                            ...
                            <div className="flex items-center gap-3 border-t border-gray-200 pt-4 dark:border-gray-700">
                                {(canCreate || canEdit) && (
                                    <Button type="submit" className="flex-1">
                                        {formMode === 'create' ? t('Add Research Category') : t('Update Research Category')}
                                    </Button>
                                )}
                        ...
                        <div className="space-y-4">
                            <div className="flex gap-2">
                                <div className="relative flex-1">
                                    ...
                                    <Input type="text" placeholder={t('Search research categories...')} value={searchTerm}
                            ...
                            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                                <Select value={selectedPracticeArea} onValueChange={setSelectedPracticeArea}>
                                    <SelectTrigger><SelectValue placeholder={t('All Practice Areas')} /></SelectTrigger>
                                    <SelectContent searchable={true}>
                                        <SelectItem value="all">{t('All Practice Areas')}</SelectItem>
                                        {(allPracticeAreas || []).map((area: any) => (
                                            <SelectItem key={area.id} value={area.id.toString()}>{area.name}</SelectItem>
                                        ))}
                                    </SelectContent>
                                </Select>
                        ...
                        {(categories?.data || []).length > 0 ? (
                            <>
                                {/* <div className="border-b border-gray-200 px-6 py-4 dark:border-gray-700">
                                    <h3 className="text-lg font-semibold text-gray-900 dark:text-white">{t('Research Categories')}</h3>
                                    <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">{t('Manage research categories for your organization.')}</p>
                                </div> */}
                                    ...
                                    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                        <thead className="bg-gray-50 dark:bg-gray-700">
                                            <tr className='bg-[#F0F0F1] hover:bg-[#F0F0F1] dark:border-gray-900 dark:bg-gray-900 border-t'>
                                                ...
                                                <th className="px-3 py-3 text-left text-xs font-medium tracking-wider text-gray-500 dark:text-gray-300">{t('Practice Area')}</th>
                                            ...
                                            {categories.data.map((item: any) => (
                                                <tr key={item.id} className="transition-colors hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                                    ...
                                                    <td className="px-3 py-4">
                                                        <span className="text-sm text-gray-700 dark:text-gray-300">{item.practice_area?.name || '-'}</span>
                                                    </td>
                                                    <td className="px-3 py-4">
                                    ...
                                    {categories.data.map((item: any) => (
                                        <div key={item.id} className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                                            ...
                                            <div className="mt-3 grid grid-cols-2 gap-4 border-t border-gray-100 pt-3 dark:border-gray-700">
                                                <div>
                                                    <p className="mb-1 text-xs text-gray-500 dark:text-gray-400">{t('Practice Area')}</p>
                                                    <span className="text-sm text-gray-700 dark:text-gray-300">{item.practice_area?.name || '-'}</span>
                                                </div>
                                                <div>
                                ...
                                {categories?.total > (categories?.per_page || 10) && (
                                    <div className="border-t border-gray-200 dark:border-gray-700">
                                        <Pagination
                                            from={categories?.from || 0} to={categories?.to || 0}
                                            total={categories?.total || 0} links={categories?.links}
                            ...
                            <div className="p-12 text-center">
                                ...
                                <h3 className="mb-2 text-lg font-medium text-gray-900 dark:text-white">{t('No research categories found')}</h3>
                                <p className="mx-auto mb-6 max-w-sm text-gray-500 dark:text-gray-400">
                                    {hasActiveFilters()
                                        ? t('No research categories match your search criteria. Try adjusting your filters.')
                                        : t('Create research categories to organize your legal research.')}
                                </p>
                                {!hasActiveFilters() && canCreate && (
                                    <p className="text-sm text-gray-500 dark:text-gray-400">{t('Use the form on the left to add your first research category.')}</p>
                                )}
                            </div>
                        )}
                    </div>
            ...
            <CrudDeleteModal
                ...
                entityName={t('research category')}
            />
        </PageTemplate>
    );
}
