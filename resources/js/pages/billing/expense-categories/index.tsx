...
import { Edit, Lock, Search, Trash2, X, Receipt, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function ExpenseCategories() {
    ...
    }, [checkOverflow, expenseCategories?.data]);
    ...
    const handleResetFilters = () => {
        ...
        router.get(route('billing.expense-categories.index'), {}, { preserveState: true, preserveScroll: true });
    };
    ...
    const handleFormSubmit = (e: React.FormEvent) => {
        ...
        if (formMode === 'create') {
            router.post(route('billing.expense-categories.store'), formData, {
                ...
                onError: (errors) => { setFormErrors(errors); toast.error(t('Failed to create expense category.')); },
            });
        } else {
            router.put(route('billing.expense-categories.update', currentItem.id), formData, {
                onSuccess: (page) => { if (page.props.flash.success) { toast.success(page.props.flash.success); resetForm(); } else if (page.props.flash.error) toast.error(page.props.flash.error); },
                onError: (errors) => { setFormErrors(errors); toast.error(t('Failed to update expense category.')); },
            });
        }
    };
    ...
    const handleDeleteConfirm = () => {
        router.delete(route('billing.expense-categories.destroy', currentItem.id), {
            ...
            onError: (errors) => { setIsDeleteModalOpen(false); toast.error(`${t('Failed to delete expense category')}: ${Object.values(errors).join(', ')}`); },
        });
    };

    const handleToggleStatus = (item: any) => {
        router.put(route('billing.expense-categories.toggle-status', item.id), {}, {
            onSuccess: (page) => {
            ...
            onError: (errors) => toast.error(`${t('Failed to update expense category')}: ${Object.values(errors).join(', ')}`),
        });
    };
    ...
    const canCreate = hasPermission(permissions, 'create-expense-categories');
    const canEdit = hasPermission(permissions, 'edit-expense-categories');
    const canToggle = hasPermission(permissions, 'toggle-status-expense-categories');
    const canDelete = hasPermission(permissions, 'delete-expense-categories');
    ...
    return (
        <PageTemplate title={t('Expense Categories')} description={t('Define and manage categories used to classify employee expenses.')} url="/billing/expense-categories" breadcrumbs={breadcrumbs} noPadding>
                    ...
                    <div className="sticky top-4 rounded-lg border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
                        <div className="border-b border-gray-200 p-6 dark:border-gray-700">
                            <h2 className="text-lg font-semibold text-gray-900 dark:text-white">
                                {formMode === 'create' ? t('Add New Expense Category') : t('Edit Expense Category')}
                            </h2>
                            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                                {formMode === 'create' ? t('Fill in the details to create a new expense category') : t('Update the expense category details below')}
                            </p>
                        </div>
                        ...
                        <form onSubmit={handleFormSubmit} className="space-y-4 p-6">
                            <div className="space-y-2">
                                ...
                                <Input id="name" type="text" value={formData.name}
                                    ...
                                    placeholder={t('eg. Travel, Office Supplies')}
                            ...
                            <div className="flex items-center gap-3 border-t border-gray-200 pt-4 dark:border-gray-700">
                                {(canCreate || canEdit) && (
                                    <Button type="submit" className="flex-1">
                                        {formMode === 'create' ? t('Add Expense Category') : t('Update Expense Category')}
                                    </Button>
                                )}
                        ...
                        <div className="space-y-4">
                            <div className="flex gap-2">
                                <div className="relative flex-1">
                                    ...
                                    <Input type="text" placeholder={t('Search expense categories...')} value={searchTerm}
                        ...
                        {(expenseCategories?.data || []).length > 0 ? (
                            <>
                                {/* <div className="border-b border-gray-200 px-6 py-4 dark:border-gray-700">
                                    <h3 className="text-lg font-semibold text-gray-900 dark:text-white">{t('Expense Categories')}</h3>
                                    <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">{t('Manage expense categories for your organization.')}</p>
                                </div> */}
                                ...
                                {expenseCategories?.total > (expenseCategories?.per_page || 10) && (
                                    <div className="border-t border-gray-200 dark:border-gray-700">
                                        <Pagination
                                            from={expenseCategories?.from || 0} to={expenseCategories?.to || 0}
                                            total={expenseCategories?.total || 0} links={expenseCategories?.links}
                            ...
                            <div className="p-12 text-center">
                                ...
                                <h3 className="mb-2 text-lg font-medium text-gray-900 dark:text-white">{t('No expense categories found')}</h3>
                                <p className="mx-auto mb-6 max-w-sm text-gray-500 dark:text-gray-400">
                                    {hasActiveFilters() ? t('No expense categories match your search criteria. Try adjusting your filters.') : t('Create expense categories to organize your billing.')}
                                </p>
                                {!hasActiveFilters() && canCreate && <p className="text-sm text-gray-500 dark:text-gray-400">{t('Use the form on the left to add your first expense category.')}</p>}
                            </div>
                        )}
                    </div>
                </div>
