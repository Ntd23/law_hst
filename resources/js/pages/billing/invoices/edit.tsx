...
import { Trash2, Plus, Clock, FileText, ArrowLeft, ScrollText } from 'lucide-react';
...
export default function EditInvoice() {
    ...
    const { clients, cases, invoice } = usePage().props as any;
    ...
    const [filteredCases, setFilteredCases] = useState(cases.filter(c => parseInt(c.client_id) === parseInt(invoice.client_id)) || []);
    ...
    return (
        <PageTemplate
            title={t('Edit Invoice')}
            description={t('Update invoice details, line items, and billing information.')}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
                        ...
                        <div>
                            ...
                            {(!clients || clients.length === 0) && (
                                <p className="text-xs mt-1">Click here to add <a href={route('clients.index')} className="underline font-medium">{t('Clients')}</a></p>
                            )}
                        </div>
                        <div>
                            ...
                            {formData.client_id && filteredCases.length === 0 && (
                                <p className="text-xs mt-1">Click here to add <a href={route('cases.index')} className="underline font-medium">{t('Cases')}</a></p>
                            )}
                        </div>
                    </div>
                        ...
                        <div className="w-72">
                            ...
                            <div className="space-y-2 text-sm">
                                <div className="flex justify-between text-gray-600">
                                    ...
                                    <span className="font-mono">{formatAmount((calculateSubtotal() || 0).toFixed(2))}</span>
                                </div>
                                ...
                                <div className="flex justify-between text-gray-600">
                                    ...
                                    <span className="font-mono">{formatAmount((calculateTaxAmount() || 0).toFixed(2))}</span>
                                </div>
                                <div className="flex justify-between text-gray-900 text-base border-t border-gray-200 pt-2">
                                    <span>{t('Total')}</span>
                                    <span className="font-mono">{formatAmount((calculateTotal() || 0).toFixed(2))}</span>
                                </div>
                            </div>
                        </div>
                    </div>
