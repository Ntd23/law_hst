...
export default function TaskTypes() {
    ...
    }, [checkOverflow, taskTypes?.data]);

    const [formData, setFormData] = useState({ name: '', description: '', color: '#3B82F6', default_duration: '', status: 'active' });
    const [formErrors, setFormErrors] = useState<any>({});

    const resetForm = () => {
        setFormData({ name: '', description: '', color: '#3B82F6', default_duration: '', status: 'active' });
    ...
    const getDuration = (minutes: number | null) => {
        if (!minutes) return '—';
        switch (minutes) {
            case 15: return t('15 min');
            case 30: return t('30 min');
            case 45: return t('45 min');
            case 60: return t('1 hr');
            case 90: return t('1.5 hrs');
            case 120: return t('2 hrs');
            case 180: return t('3 hrs');
            case 240: return t('4 hrs');
            case 480: return t('8 hrs');
    ...
    const handleResetFilters = () => {
        ...
        router.get(route('tasks.task-types.index'), {}, { preserveState: true, preserveScroll: true });
    };
    ...
    const handleFormSubmit = (e: React.FormEvent) => {
        ...
        const payload = { ...formData, default_duration: formData.default_duration ? Number(formData.default_duration) : null };
        if (formMode === 'create') {
            router.post(route('tasks.task-types.store'), payload, {
                ...
                onError: (errors) => { setFormErrors(errors); toast.error(t('Failed to create task type.')); },
            });
        } else {
            router.put(route('tasks.task-types.update', currentItem.id), payload, {
                onSuccess: (page) => { if (page.props.flash.success) { toast.success(page.props.flash.success); resetForm(); } else if (page.props.flash.error) toast.error(page.props.flash.error); },
                onError: (errors) => { setFormErrors(errors); toast.error(t('Failed to update task type.')); },
            });
        }
    };
    ...
    const handleDeleteConfirm = () => {
        router.delete(route('tasks.task-types.destroy', currentItem.id), {
            ...
            onError: (errors) => { setIsDeleteModalOpen(false); toast.error(`${t('Failed to delete task type')}: ${Object.values(errors).join(', ')}`); },
        });
    };

    const handleToggleStatus = (item: any) => {
        router.put(route('tasks.task-types.toggle-status', item.id), {}, {
            onSuccess: (page) => {
            ...
            onError: (errors) => toast.error(`${t('Failed to update task type')}: ${Object.values(errors).join(', ')}`),
        });
    };
    ...
    const canCreate = hasPermission(permissions, 'create-task-types');
    const canEdit = hasPermission(permissions, 'edit-task-types');
    const canToggle = hasPermission(permissions, 'toggle-status-task-types');
    const canDelete = hasPermission(permissions, 'delete-task-types');
    ...
    return (
        <PageTemplate title={t('Task Types')} description={t('Define and manage task type classifications.')} url="/tasks/task-types" breadcrumbs={breadcrumbs} noPadding>
                    ...
                    <div className="sticky top-4 rounded-lg border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
                        <div className="border-b border-gray-200 p-6 dark:border-gray-700">
                            <h2 className="text-lg font-semibold text-gray-900 dark:text-white">
                                {formMode === 'create' ? t('Add New Task Type') : t('Edit Task Type')}
                            </h2>
                            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                                {formMode === 'create' ? t('Fill in the details to create a new task type') : t('Update the task type details below')}
                            </p>
                        </div>

                        <form onSubmit={handleFormSubmit} className="space-y-4 p-6">
                            <div className="space-y-2">
                                <Label htmlFor="name" required>{t('Task Type Name')}</Label>
                                <Input id="name" type="text" value={formData.name}
                                    onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                                    placeholder={t('eg. Research, Drafting')}
                            ...
                            <div className="space-y-2">
                                ...
                                <Textarea id="description" value={formData.description}
                                    ...
                                    placeholder={t('eg. Tasks involving legal research')}
                            ...
                            <div className="space-y-2">
                                <Label htmlFor="default_duration">{t('Default Duration (minutes)')}</Label>
                                <Input id="default_duration" type="number" min="1" value={formData.default_duration}
                            ...
                            <div className="flex items-center gap-3 border-t border-gray-200 pt-4 dark:border-gray-700">
                                {(canCreate || canEdit) && (
                                    <Button type="submit" className="flex-1">
                                        {formMode === 'create' ? t('Add Task Type') : t('Update Task Type')}
                                    </Button>
                                )}
                        ...
                        <div className="space-y-4">
                            <div className="flex gap-2">
                                <div className="relative flex-1">
                                    ...
                                    <Input type="text" placeholder={t('Search task types...')} value={searchTerm}
                        ...
                        {(taskTypes?.data || []).length > 0 ? (
                            <>
                                {/* <div className="border-b border-gray-200 px-6 py-4 dark:border-gray-700">
                                    <h3 className="text-lg font-semibold text-gray-900 dark:text-white">{t('Task Types')}</h3>
                                    <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">{t('Manage task type categories for your organization.')}</p>
                                </div> */}
                                    ...
                                    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                        <thead className="bg-gray-50 dark:bg-gray-700">
                                            <tr className='bg-[#F0F0F1] hover:bg-[#F0F0F1] dark:border-gray-900 dark:bg-gray-900 border-t'>
                                                ...
                                                <th className="cursor-pointer px-3 py-3 text-left text-xs font-medium tracking-wider text-gray-500 select-none dark:text-gray-300" onClick={() => handleSort('default_duration')}>
                                                    <div className="flex items-center gap-1">
                                                        {t('Duration')}
                                                        {pageFilters.sort_field === 'default_duration' ? (pageFilters.sort_direction === 'asc' ? ' ↑' : ' ↓') : <span className="opacity-40">↕</span>}
                                                    </div>
                                                </th>
                                            ...
                                            {taskTypes.data.map((item: any) => (
                                                <tr key={item.id} className="transition-colors hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                                    ...
                                                    <td className="px-3 py-4 text-sm text-gray-700 dark:text-gray-300">
                                                        {getDuration(item.default_duration)}
                                                    </td>
                                        ...
                                        <div key={item.id} className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                                            ...
                                            <div className="mt-3 grid grid-cols-2 gap-3 border-t border-gray-100 pt-3 dark:border-gray-700">
                                                <div>
                                                    ...
                                                    <span className="text-sm text-gray-700 dark:text-gray-300">{getDuration(item.default_duration)}</span>
                                                </div>
                                                <div>
                                ...
                                {taskTypes?.total > (taskTypes?.per_page || 10) && (
                                    <div className="border-t border-gray-200 dark:border-gray-700">
                                        <Pagination
                                            from={taskTypes?.from || 0} to={taskTypes?.to || 0}
                                            total={taskTypes?.total || 0} links={taskTypes?.links}
                            ...
                            <div className="p-12 text-center">
                                ...
                                <h3 className="mb-2 text-lg font-medium text-gray-900 dark:text-white">{t('No task types found')}</h3>
                                <p className="mx-auto mb-6 max-w-sm text-gray-500 dark:text-gray-400">
                                    {hasActiveFilters() ? t('No task types match your search criteria. Try adjusting your filters.') : t('Create task types to categorize your tasks.')}
                                </p>
                                {!hasActiveFilters() && canCreate && <p className="text-sm text-gray-500 dark:text-gray-400">{t('Use the form on the left to add your first task type.')}</p>}
                            </div>
                        )}
                    </div>
                </div>
