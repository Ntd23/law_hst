...
import { Edit, Lock, Search, Trash2, X, CalendarClock, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function EventTypes() {
    ...
    const handleResetFilters = () => {
        ...
        router.get(route('advocate.event-types.index'), {}, { preserveState: true, preserveScroll: true });
    };
    ...
    const handleFormSubmit = (e: React.FormEvent) => {
        ...
        if (formMode === 'create') {
            router.post(route('advocate.event-types.store'), formData, {
                ...
                onError: (errors) => { setFormErrors(errors); toast.error(t('Failed to create event type.')); },
            });
        } else {
            router.put(route('advocate.event-types.update', currentItem.id), formData, {
                onSuccess: (page) => { if (page.props.flash.success) { toast.success(page.props.flash.success); resetForm(); } else if (page.props.flash.error) toast.error(page.props.flash.error); },
                onError: (errors) => { setFormErrors(errors); toast.error(t('Failed to update event type.')); },
            });
        }
    };
    ...
    const handleDeleteConfirm = () => {
        router.delete(route('advocate.event-types.destroy', currentItem.id), {
            ...
            onError: (errors) => { setIsDeleteModalOpen(false); toast.error(`${t('Failed to delete event type')}: ${Object.values(errors).join(', ')}`); },
        });
    };
    ...
    const handleToggleStatus = (item: any) => {
        router.put(route('advocate.event-types.toggle-status', item.id), {}, {
            ...
            onError: (errors) => toast.error(`${t('Failed to update event type')}: ${Object.values(errors).join(', ')}`),
        });
    };
    ...
    const canCreate = hasPermission(permissions, 'create-event-types');
    const canEdit = hasPermission(permissions, 'edit-event-types');
    const canToggle = hasPermission(permissions, 'toggle-status-event-types');
    const canDelete = hasPermission(permissions, 'delete-event-types');
    ...
    return (
        <PageTemplate title={t('Event Types')} url="/advocate/event-types" description={t('Manage event types for your organization.')} breadcrumbs={breadcrumbs} noPadding>
                    ...
                    <div className="sticky top-4 rounded-lg border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
                        <div className="border-b border-gray-200 p-6 dark:border-gray-700">
                            <h2 className="text-lg font-semibold text-gray-900 dark:text-white">
                                {formMode === 'create' ? t('Add New Event Type') : t('Edit Event Type')}
                            </h2>
                            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                                {formMode === 'create' ? t('Fill in the details to create a new event type') : t('Update the event type details below')}
                            </p>
                        </div>

                        <form onSubmit={handleFormSubmit} className="space-y-4 p-6">
                            <div className="space-y-2">
                                <Label htmlFor="name" required>{t('Event Type Name')}</Label>
                                <Input id="name" type="text" value={formData.name}
                                    onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                                    placeholder={t('eg. Hearing, Conference')}
                            ...
                            <div className="space-y-2">
                                ...
                                <Textarea id="description" value={formData.description}
                                    ...
                                    placeholder={t('Enter event type description...')}
                            ...
                            <div className="flex items-center gap-3 border-t border-gray-200 pt-4 dark:border-gray-700">
                                {(canCreate || canEdit) && (
                                    <Button type="submit" className="flex-1">
                                        {formMode === 'create' ? t('Add Event Type') : t('Update Event Type')}
                                    </Button>
                                )}
                        ...
                        <div className="space-y-4">
                            <div className="flex gap-2">
                                <div className="relative flex-1">
                                    ...
                                    <Input type="text" placeholder={t('Search event types...')} value={searchTerm}
                        ...
                        {(eventTypes?.data || []).length > 0 ? (
                            <>
                                {/* <div className="border-b border-gray-200 px-6 py-4 dark:border-gray-700">
                                    <h3 className="text-lg font-semibold text-gray-900 dark:text-white">{t('Event Types')}</h3>
                                    <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">{t('Manage event type categories for your organization.')}</p>
                                </div> */}
                                ...
                                {eventTypes?.total > (eventTypes?.per_page || 10) && (
                                    <div className="border-t border-gray-200 dark:border-gray-700">
                                        <Pagination
                                            from={eventTypes?.from || 0} to={eventTypes?.to || 0}
                                            total={eventTypes?.total || 0} links={eventTypes?.links}
                            ...
                            <div className="p-12 text-center">
                                ...
                                <h3 className="mb-2 text-lg font-medium text-gray-900 dark:text-white">{t('No event types found')}</h3>
                                <p className="mx-auto mb-6 max-w-sm text-gray-500 dark:text-gray-400">
                                    {hasActiveFilters() ? t('No event types match your search criteria. Try adjusting your filters.') : t('Create event types to categorize your events.')}
                                </p>
                                {!hasActiveFilters() && canCreate && <p className="text-sm text-gray-500 dark:text-gray-400">{t('Use the form on the left to add your first event type.')}</p>}
                            </div>
                        )}
                    </div>
                </div>
