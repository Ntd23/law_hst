...
export default function AboutSection({ data, setData, errors, handleInputChange, getSectionData, updateSectionData, updateSectionVisibility, t = (key) => key }) {
  ...
  const convertToRelativePath = (url: string): string => {
    ...
    // Handle images folder paths
    const imagesIndex = url.indexOf('/images/');
    if (imagesIndex !== -1) {
      return url.substring(imagesIndex);
    }

    // If it's already a relative path or unknown format, return as is
    return url;
  };
  return (
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-3">
            <MediaPicker
              label={t("About Image")}
              value={getImagePath(getSectionData('about').image || '')}
              onChange={(value) => {
                const relativePath = convertToRelativePath(value);
                updateSectionData('about', { image: relativePath });
              }}
