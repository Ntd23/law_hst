...
import { Edit, Lock, Search, Trash2, X, ChevronDown, ChevronUp, CheckSquare } from 'lucide-react';
...
export default function CaseStatuses() {
    ...
    }, [checkOverflow, caseStatuses?.data]);

    const [formData, setFormData] = useState({
        name: '', description: '', color: '#10b77f',
        is_default: false, is_closed: false, status: 'active',
    });
    const [formErrors, setFormErrors] = useState<any>({});

    const resetForm = () => {
        setFormData({ name: '', description: '', color: '#10b77f', is_default: false, is_closed: false, status: 'active' });
        setFormErrors({});
        setFormMode('create');
        setCurrentItem(null);
    };
    ...
    const applyFilters = () => {
        router.get(route('cases.case-statuses.index'), {
            page: 1,
    ...
    const handleResetFilters = () => {
        ...
        router.get(route('cases.case-statuses.index'), {}, { preserveState: true, preserveScroll: true });
    };
    ...
    const handleFormSubmit = (e: React.FormEvent) => {
        ...
        if (formMode === 'create') {
            router.post(route('cases.case-statuses.store'), formData, {
                ...
                onError: (errors) => { setFormErrors(errors); toast.error(`Failed to create case status: ${Object.values(errors).join(', ')}`); },
            });
        } else {
            router.put(route('cases.case-statuses.update', currentItem.id), formData, {
                ...
                onError: (errors) => { setFormErrors(errors); toast.error(`Failed to update case status: ${Object.values(errors).join(', ')}`); },
            });
        }
    };
    ...
    const handleDeleteConfirm = () => {
        router.delete(route('cases.case-statuses.destroy', currentItem.id), {
            ...
            onError: (errors) => { setIsDeleteModalOpen(false); toast.error(`Failed to delete case status: ${Object.values(errors).join(', ')}`); },
        });
    };

    const handleToggleStatus = (caseStatus: any) => {
        router.put(route('cases.case-statuses.toggle-status', caseStatus.id), {}, {
            onSuccess: (page) => {
                if (page.props.flash.success) {
                    toast.success(page.props.flash.success);
                    if (formMode === 'edit' && currentItem?.id === caseStatus.id)
                        setFormData(prev => ({ ...prev, status: caseStatus.status === 'active' ? 'inactive' : 'active' }));
                } else if (page.props.flash.error) toast.error(page.props.flash.error);
            },
            onError: (errors) => { toast.error(`Failed to update case status: ${Object.values(errors).join(', ')}`); },
        });
    };
    ...
    const canCreate = hasPermission(permissions, 'create-case-statuses');
    const canEdit = hasPermission(permissions, 'edit-case-statuses');
    const canToggle = hasPermission(permissions, 'toggle-status-case-statuses');
    const canDelete = hasPermission(permissions, 'delete-case-statuses');
    ...
    return (
        <PageTemplate title={t('Case Status')} url="/cases/case-statuses" description={t('Manage case statuses for your cases.')} breadcrumbs={breadcrumbs} noPadding>
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
                    ...
                    <div className="sticky top-4 rounded-lg border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
                        <div className="border-b border-gray-200 p-6 dark:border-gray-700">
                            <h2 className="text-lg font-semibold text-gray-900 dark:text-white">
                                {formMode === 'create' ? t('Add New Case Status') : t('Edit Case Status')}
                            </h2>
                            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                                {formMode === 'create' ? t('Fill in the details to create a new case status') : t('Update the case status details below')}
                            </p>
                        </div>
                        ...
                        <form onSubmit={handleFormSubmit} className="space-y-4 p-6">
                            ...
                            <div className="space-y-2">
                                ...
                                <Textarea id="description" value={formData.description}
                                    ...
                                    placeholder={t('eg. Case is currently being worked on')}
                            ...
                            <div className="flex items-center gap-6">
                                <label className="flex items-center gap-2 cursor-pointer">
                                    <Checkbox
                                        ...
                                        disabled={!canCreate && !canEdit} />
                                    <span className="text-sm text-gray-700 dark:text-gray-300">{t('Default Status')}</span>
                                </label>
                                <label className="flex items-center gap-2 cursor-pointer">
                                    <Checkbox
                                        ...
                                        disabled={!canCreate && !canEdit} />
                                    <span className="text-sm text-gray-700 dark:text-gray-300">{t('Closed Status')}</span>
                                </label>
                            </div>
                            ...
                            <div className="flex items-center gap-3 border-t border-gray-200 pt-4 dark:border-gray-700">
                                {(canCreate || canEdit) && (
                                    <Button type="submit" className="flex-1">
                                        {formMode === 'create' ? t('Add Case Status') : t('Update Case Status')}
                                    </Button>
                                )}
                ...
                <div className="space-y-4 lg:col-span-2">
                        ...
                        <div className="space-y-4">
                            <div className="flex gap-2">
                                <div className="relative flex-1">
                                    ...
                                    <Input type="text" placeholder={t('Search case statuses...')} value={searchTerm}
                    ...
                    <div className="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
                        {(caseStatuses?.data || []).length > 0 ? (
                            <>
                                {/* <div className="border-b border-gray-200 px-6 py-4 dark:border-gray-700">
                                    <h3 className="text-lg font-semibold text-gray-900 dark:text-white">{t('Case Statuses')}</h3>
                                    <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">{t('Manage case statuses for your organization.')}</p>
                                </div> */}
                                    ...
                                    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                        <thead className="bg-gray-50 dark:bg-gray-700">
                                            <tr className='bg-[#F0F0F1] hover:bg-[#F0F0F1] dark:border-gray-900 dark:bg-gray-900 border-t'>
                                                ...
                                                <th className="px-3 py-3 text-left text-xs font-medium tracking-wider text-gray-500 dark:text-gray-300">{t('Default')}</th>
                                                <th className="px-3 py-3 text-left text-xs font-medium tracking-wider text-gray-500 dark:text-gray-300">{t('Closed Status')}</th>
                                        ...
                                        <tbody className="divide-y divide-gray-200 bg-white dark:divide-gray-700 dark:bg-gray-800">
                                            {caseStatuses.data.map((item: any) => (
                                                <tr key={item.id} className="transition-colors hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                                    <td className="px-4 py-4">
                                                        <div className="flex items-center">
                                                            <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-lg text-white" style={{ backgroundColor: item.color || '#10b77f' }}>
                                                    ...
                                                    <td className="px-3 py-4">
                                                        <span className={`inline-flex items-center rounded-md px-2 py-1 text-xs font-medium ring-1 ring-inset ${item.is_default ? 'bg-blue-50 text-blue-700 ring-blue-600/20' : 'bg-gray-50 text-gray-700 ring-gray-600/20'}`}>
                                                            {item.is_default ? t('Yes') : t('No')}
                                                        </span>
                                                    </td>
                                                    <td className="px-3 py-4">
                                                        <span className={`inline-flex items-center rounded-md px-2 py-1 text-xs font-medium ring-1 ring-inset ${item.is_closed ? 'bg-red-50 text-red-700 ring-red-600/20' : 'bg-green-50 text-green-700 ring-green-600/20'}`}>
                                    ...
                                    {caseStatuses.data.map((item: any) => (
                                        <div key={item.id} className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                                            ...
                                            <div className="mt-3 grid grid-cols-2 gap-3 border-t border-gray-100 pt-3 dark:border-gray-700">
                                                <div>
                                                    <p className="mb-1 text-xs text-gray-500 dark:text-gray-400">{t('Default')}</p>
                                                    <span className={`inline-flex items-center rounded-md px-2 py-1 text-xs font-medium ring-1 ring-inset ${item.is_default ? 'bg-blue-50 text-blue-700 ring-blue-600/20' : 'bg-gray-50 text-gray-700 ring-gray-600/20'}`}>
                                                        {item.is_default ? t('Yes') : t('No')}
                                                    </span>
                                                </div>
                                                <div>
                                                    <p className="mb-1 text-xs text-gray-500 dark:text-gray-400">{t('Closed Status')}</p>
                                                    <span className={`inline-flex items-center rounded-md px-2 py-1 text-xs font-medium ring-1 ring-inset ${item.is_closed ? 'bg-red-50 text-red-700 ring-red-600/20' : 'bg-green-50 text-green-700 ring-green-600/20'}`}>
                                                        {item.is_closed ? t('Closed') : t('Open')}
                                                    </span>
                                                </div>
                                ...
                                {caseStatuses?.total > (caseStatuses?.per_page || 10) && (
                                    <div className="border-t border-gray-200 dark:border-gray-700">
                                        <Pagination
                                            from={caseStatuses?.from || 0} to={caseStatuses?.to || 0}
                                            total={caseStatuses?.total || 0} links={caseStatuses?.links}
                            ...
                            <div className="p-12 text-center">
                                ...
                                <h3 className="mb-2 text-lg font-medium text-gray-900 dark:text-white">{t('No case statuses found')}</h3>
                                <p className="mx-auto mb-6 max-w-sm text-gray-500 dark:text-gray-400">
                                    {hasActiveFilters()
                                        ? t('No case statuses match your search criteria. Try adjusting your filters.')
                                        : t('Create case statuses to manage the lifecycle of your cases.')}
                                </p>
                                {!hasActiveFilters() && canCreate && (
                                    <p className="text-sm text-gray-500 dark:text-gray-400">{t('Use the form on the left to add your first case status.')}</p>
                                )}
                            </div>
                        )}
                    </div>
