...
import { FileText, Tag, Lock, Calendar, Shield, Eye } from 'lucide-react';
...
export default function DocumentView({ record, documentTypes }: ViewProps) {
    const { t } = useTranslation();

    const confidentialityColors: Record<string, string> = {
        public: 'bg-emerald-50 text-emerald-700 ring-emerald-600/20',
        confidential: 'bg-yellow-50 text-yellow-700 ring-yellow-600/20',
        privileged: 'bg-red-50 text-red-700 ring-red-600/20',
    };

    const docTypeName = record?.document_type?.name;
            ...
            <div className="px-6 py-4 pb-6 space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <p className="mt-1 text-sm font-medium text-gray-900 dark:text-white">{docTypeName}</p>
                    </div>
                </div>

                {/* Confidentiality & Document Date */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label className="text-sm font-medium text-gray-500 flex items-center gap-2">
                        ...
                        <div className="mt-1">
                            <span className={`inline-flex items-center rounded-md px-2 py-1 text-xs font-medium ring-1 ring-inset ${confidentialityColors[record?.confidentiality] || 'bg-gray-50 text-gray-700 ring-gray-600/20'
                                }`}>
                                {record?.confidentiality ? record.confidentiality.charAt(0).toUpperCase() + record.confidentiality.slice(1) : '-'}
                            </span>
                        </div>
                    </div>
                    <div>
                        ...
                        <p className="mt-1 text-sm font-medium text-gray-900 dark:text-white">
                            {record?.document_date ? (window.appSettings?.formatDate(record.document_date) || new Date(record.document_date).toLocaleDateString()) : '-'}
                        </p>
                    </div>
                </div>
